...
public class KafkaConfigurationConsumer {
	...
	@Bean
	public ConcurrentKafkaListenerContainerFactory<String, String> purchaseRequestListener() {
	...
	/*
	@Bean
	public ConsumerFactory<String, PurchaseRequest> consumerJsonFactory() {
		Map<String, Object> config = new HashMap<>();
		config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
		config.put(ConsumerConfig.GROUP_ID_CONFIG, "grupo01");
		config.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
	...
	@Bean
	public ConcurrentKafkaListenerContainerFactory<String, PurchaseRequest> purchaseRequestJsonListener() {
